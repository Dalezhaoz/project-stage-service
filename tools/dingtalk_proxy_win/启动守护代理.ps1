...
if ([string]::IsNullOrWhiteSpace($exePath)) {
    ...
    [System.Windows.Forms.MessageBox]::Show("找不到 dist_new 或 dist 下的 DingTalkProxy.exe，请先编译或发布。", "DingTalkProxy")
    exit 1
}
...
while ($true) {
    $now = Get-Date
    Add-Content -Path $launcherLog -Value ("[{0}] starting proxy" -f $now.ToString("yyyy-MM-dd HH:mm:ss"))
