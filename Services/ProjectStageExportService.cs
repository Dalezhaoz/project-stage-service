...
public sealed class ProjectStageExportService
{
    public byte[] Export(ProjectStageSummary summary)
    {
        ...
        sheet.Cell(1, 3).Value = "考试代码";
        sheet.Cell(1, 4).Value = "项目名称";
        sheet.Cell(1, 5).Value = "阶段名称";
        sheet.Cell(1, 6).Value = "开始时间";
        sheet.Cell(1, 7).Value = "结束时间";
        sheet.Cell(1, 8).Value = "当前状态";
        ...
        foreach (var item in summary.Records)
        {
            ...
            sheet.Cell(row, 3).Value = item.ExamCode;
            sheet.Cell(row, 4).Value = item.ProjectName;
            sheet.Cell(row, 5).Value = item.StageName;
            sheet.Cell(row, 6).Value = item.StartTime;
            sheet.Cell(row, 7).Value = item.EndTime;
