...
app.MapPost("/api/dingtalk/test-personal", async (HttpContext httpContext, DingTalkNotifyService notifyService, SummaryStoreConfigStore summaryStoreConfigStore, LocalAuthService authService, ScheduleConfigStore scheduleConfigStore, CancellationToken cancellationToken) =>
{
    ...
    try
    {
        ...
        var isAdminOrInternal = string.Equals(me.Role, "admin", StringComparison.OrdinalIgnoreCase)
            || string.Equals(me.Role, "internal", StringComparison.OrdinalIgnoreCase);
        if (isAdminOrInternal)
        {
            // admin/internal 测试推送发全量报告
...
app.MapPost("/api/dingtalk/test-nextday", async (DingTalkNotifyService notifyService, SummaryStoreConfigStore summaryStoreConfigStore, ScheduleConfigStore scheduleConfigStore, LocalAuthService authService, CancellationToken cancellationToken) =>
{
    try
    {
        var scheduleConfig = await scheduleConfigStore.LoadAsync(cancellationToken);
        ...
        var result = await notifyService.SendNextDayPreviewAsync(summaryConfig, scheduleConfig.DingTalkConfig, userConfigs, cancellationToken);
        return Results.Ok(new
        {
            sent = result.MainSent || result.SentUsers.Count > 0,
            totalStages = result.TotalStages,
...
app.MapPost("/api/dingtalk/test-user-nextday", async (TestUserDingTalkRequest request, DingTalkNotifyService notifyService, SummaryStoreConfigStore summaryStoreConfigStore, ScheduleConfigStore scheduleConfigStore, LocalAuthService authService, CancellationToken cancellationToken) =>
{
    try
    {
        var users = await authService.GetUsersAsync(cancellationToken);
        ...
        if (isAdminOrInternal)
        {
            ...
            var result = await notifyService.SendNextDayPreviewAsync(summaryConfig, mainConfig, [], cancellationToken);
            return Results.Ok(new { sent = result.MainSent, totalStages = result.TotalStages });
        }
        else
        {
            ...
            var dummyMainConfig = new DingTalkConfig { ProxyUrl = proxyUrl };
            var result = await notifyService.SendNextDayPreviewAsync(summaryConfig, dummyMainConfig, userConfigs, cancellationToken);

            if (result.TotalStages == 0)
            {
                var tomorrow = DateTime.Today.AddDays(1).ToString("yyyy-MM-dd");
                await notifyService.SendDirectMessageAsync(targetDingTalk, $"明日项目提醒 ({tomorrow})", $"### ✅ 明日无即将开始的项目\n> 日期：**{tomorrow}**", cancellationToken);
                return Results.Ok(new { sent = true, totalStages = 0 });
            }
            if (result.SkippedUsers.Contains(target.Username))
            {
                var tomorrow = DateTime.Today.AddDays(1).ToString("yyyy-MM-dd");
                await notifyService.SendDirectMessageAsync(targetDingTalk, $"明日项目提醒 ({tomorrow})", $"### ✅ 明日有 {result.TotalStages} 个阶段开始，但没有分配给 {target.Username} 的项目\n> 日期：**{tomorrow}**", cancellationToken);
                return Results.Ok(new { sent = true, totalStages = result.TotalStages });
            }
            if (result.SentUsers.Contains(target.Username))
                return Results.Ok(new { sent = true, totalStages = result.TotalStages });
